...
//   geomMeanLongSun?: number;
//   geomMeanAnomSun?: number;
//   EccentEarthOrbit?: number;
//   SunEqofCtr?: number;
//   SunTrueLong?: number;
//   SunAppLong?: number;
//   MeanObliqEcliptic?: number;
//   ObliqCorr?: number;
//   SunDeclin?: number;
//   ha?: number;
//   VarY?: number;
...
export default function calculateSunrise(date) {
    ...
    d.geomMeanLongSun =
        (280.46646 +
            d.julianCentury * (36000.76983 + d.julianCentury * 0.0003032)) %
        360;
    d.geomMeanAnomSun =
        357.52911 + d.julianCentury * (35999.05029 - 0.0001537 * d.julianCentury);
    d.EccentEarthOrbit =
        0.016708634 -
        d.julianCentury * (0.000042037 + 0.0000001267 * d.julianCentury);
    d.SunEqofCtr =
        Math.sin(radians(d.geomMeanAnomSun)) *
        (1.914602 - d.julianCentury * (0.004817 + 0.000014 * d.julianCentury)) +
        Math.sin(radians(2 * d.geomMeanAnomSun)) *
        (0.019993 - 0.000101 * d.julianCentury) +
        Math.sin(radians(3 * d.geomMeanAnomSun)) * 0.000289;
    d.SunTrueLong = d.geomMeanLongSun + d.SunEqofCtr;
    d.SunAppLong =
        d.SunTrueLong -
        0.00569 -
        0.00478 * Math.sin(radians(125.04 - 1934.136 * d.julianCentury));
    d.MeanObliqEcliptic =
        23 +
        (26 +
            (21.448 -
                d.julianCentury *
                (46.815 + d.julianCentury * (0.00059 - d.julianCentury * 0.001813))) /
            60) /
        60;
    d.ObliqCorr =
        d.MeanObliqEcliptic +
        0.00256 * Math.cos(radians(125.04 - 1934.136 * d.julianCentury));
    d.SunDeclin = degrees(
        Math.asin(Math.sin(radians(d.ObliqCorr)) * Math.sin(radians(d.SunAppLong)))
    );
    d.ha = degrees(
        Math.acos(
    ...
    d.VarY =
        Math.tan(radians(d.ObliqCorr / 2)) * Math.tan(radians(d.ObliqCorr / 2));
    d.EqOfTime =
        4 *
        degrees(
            d.VarY * Math.sin(2 * radians(d.geomMeanLongSun)) -
            2 * d.EccentEarthOrbit * Math.sin(radians(d.geomMeanAnomSun)) +
            4 *
            d.EccentEarthOrbit *
            d.VarY *
            Math.sin(radians(d.geomMeanAnomSun)) *
            Math.cos(2 * radians(d.geomMeanLongSun)) -
            0.5 * d.VarY * d.VarY * Math.sin(4 * radians(d.geomMeanLongSun)) -
            1.25 *
            d.EccentEarthOrbit *
            d.EccentEarthOrbit *
            Math.sin(2 * radians(d.geomMeanAnomSun))
        );
    ...
    d.sunriseTimestamp = Math.floor(d.sunriseTime.getTime() / 1000);
    d.sunsetTimestamp = Math.floor(d.sunsetTime.getTime() / 1000);
    return d;
}
