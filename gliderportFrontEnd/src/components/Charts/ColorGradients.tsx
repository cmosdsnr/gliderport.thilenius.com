...
/**
 * Predefined color palettes for heatmap gradients, indexed by wind-condition quality.
 *
 * Each inner array contains five stop colors ordered from highest wind speed (index 0)
 * to lightest / most favourable conditions (index 4).
 *
 * | Index | Quality | Description                          |
 * |-------|---------|--------------------------------------|
 * | 0     | good    | Vibrant colours — ideal conditions   |
 * | 1     | poor    | Muted / desaturated — marginal       |
 * | 2     | bad     | Grey-ish — unflyable                 |
 */
...
/**
 * Appends SVG `<linearGradient>` definitions (main palettes and transitions) to a `<defs>` element.
 *
 * ### Main gradients (`mg0`, `mg1`, `mg2`)
 * One vertical gradient per palette in `colors`.  Stop positions are computed from `dataMax`
 * so that colour bands align with absolute wind-speed thresholds (6, 9, 13, 19 m/s).
 *
 * ### Transitional gradients (`gd{pair}-{step}`, 0–4)
 * For each of six palette-pair transitions, five intermediate gradients are generated by
 * blending the two endpoint palettes at fractions 1/6 … 5/6.  These are painted as thin
 * 2-pixel-wide columns by `WindChart` to produce a smooth cross-fade between states.
 *
 * @param svgDefs - D3 selection of the `<defs>` element to append gradient nodes into.
 * @param dataMax - Maximum wind-speed value in the dataset; controls gradient stop offsets.
 *
 * @example
 * ```ts
 * const svgDefs = svg.append('defs');
 * getGradients(svgDefs, 25);   // paints stops for a 25 m/s maximum
 * ```
 */
