 ...
 * Called by a cron job 'Hourly' and executes every 5 minutes (10 ticks)
...
function insertOrUpdate($ts, $data)
{
    global $db;
    $d = json_encode($data);
    $db->query("INSERT INTO `code_history` SET date=$ts, data='$d' ON DUPLICATE KEY UPDATE data = '$d';");
}
...
function safeFetch($ts)
{
    ...
    if ($query->rowCount() > 0) {
        ...
        if (!is_object($obj) || !property_exists($obj, 'codes')) $obj = initialize($ts);
    } else {
        $obj = initialize($ts);
        insertOrUpdate($ts, $obj);
    }
    return ($obj);
}
...
$tsLast = 0;
$query = $db->query("SELECT * FROM `code_history` WHERE date=0;");
if ($query->rowCount() > 0) $tsLast = json_decode($query->fetch()->data);

//load today's data
$today = safeFetch($tsToday);
$day = $today->codes;
...
if (count($day) == 0) echo "there is no code data for $tsToday\n";
else echo $i . ": {$day[$i][0]} with code {$day[$i][1]} \n";
...
// var_dump($record);
insertOrUpdate($tsTomorrow, $record);
print "Added tomorrow " . $tsTomorrow . " with data " . json_encode($record) . "\n";


$next = [];
$tsTwoDays = (new DateTime("now", $tz))->add(new DateInterval('P2D'))->setTime(0, 0, 0)->getTimestamp();
...
$next[] = [$sr - 3600 * $start, $forecast[$i - 1][1]];
while (($i < count($forecast)) && ($forecast[$i][0] < $f[1])) {
    $next[] = [$forecast[$i][0] - $tsTwoDays - 3600 * $start, $forecast[$i][1]];
    $i++;
}
$next[] = [$ss - 3600 * $start, 0];
...
insertOrUpdate($tsTwoDays, $record);
print "Added day after tomorrow " . $tsTwoDays . " with data " . json_encode($record) . "\n";
